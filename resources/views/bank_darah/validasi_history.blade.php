...
<head>
    ...
    <title>Karsa ERM | Riwayat Validasi Darah</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .user-badge span { font-size: 12px; font-weight: 600; }
        ...
        .stat-icon.red { box-shadow: 0 2px 8px var(--primary-glow); }
        ...
        .filter-group input:hover, .filter-group select:hover { border-color: var(--text-subtle); }
        ...
        nav[role="navigation"] { padding: 16px 22px; border-top: 1px solid var(--border); background: var(--bg-main); }
    ...
    <div class="main-content">
        <div class="page-header">
            <div class="page-header-left">
                <h1><i class="fas fa-clipboard-check"></i> Riwayat Validasi Darah</h1>
                <p><i class="fas fa-database" style="font-size:12px;margin-right:4px;color:var(--text-subtle);"></i> Riwayat permintaan dan validasi transfusi darah</p>
            </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-info">
                    <h3>{{ number_format($totalValidated, 0, ',', '.') }}</h3>
                    <p>Sudah Validasi Transfusi</p>
                </div>
            </div>
            <div class="stat-card">
        ...
        <div class="filter-card">
            ...
            <div class="filter-body">
                <form method="GET" action="{{ route('bank-darah.validasi.history') }}">
                    <div class="filter-row">
                        <div class="filter-group"><label><i class="fas fa-calendar-alt" style="margin-right:3px;"></i> Tgl. Pemberian Awal</label><input type="date" name="tgl_awal" value="{{ request('tgl_awal', date('Y-m-d')) }}"></div>
                        <div class="filter-group"><label><i class="fas fa-calendar-check" style="margin-right:3px;"></i> Tgl. Pemberian Akhir</label><input type="date" name="tgl_akhir" value="{{ request('tgl_akhir', date('Y-m-d')) }}"></div>
                        <div class="filter-group">
                            <label><i class="fas fa-hospital" style="margin-right:3px;"></i> Ruangan</label>
                            <select name="ruangan">
                        ...
                        <div class="filter-group">
                            <label><i class="fas fa-tag" style="margin-right:3px;"></i> Status</label>
                            <select name="status">
                        ...
                        <div class="filter-group" style="flex:1;min-width:180px;">
                            ...
                            <input type="text" name="search" placeholder="Nama pasien, No. RM, atau barcode..." value="{{ request('search') }}">
                        </div>
                        <div class="filter-actions">
                            <button type="submit" class="btn-filter"><i class="fas fa-search"></i> Filter</button>
                            <a href="{{ route('bank-darah.validasi.history') }}" class="btn-filter-reset"><i class="fas fa-undo"></i> Reset</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="table-card">
            ...
            <div class="table-wrap">
                <table id="historyTable">
                    <thead>
                        <tr>
                            <th><i class="fas fa-fingerprint"></i> No. RM</th>
                            <th><i class="fas fa-user"></i> Nama Pasien</th>
                            <th><i class="fas fa-door-open"></i> Ruangan</th>
                            <th><i class="fas fa-tint"></i> Gol. Darah</th>
                            <th><i class="fas fa-flask"></i> Produk</th>
                            <th><i class="fas fa-barcode"></i> Barcode</th>
                            <th><i class="fas fa-vial"></i> Hasil Cross</th>
                            <th><i class="fas fa-calendar"></i> Tgl. Permintaan</th>
                            <th><i class="fas fa-flag"></i> Status</th>
                            <th><i class="fas fa-syringe"></i> Status Pemberian</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($data as $item)
                            <tr>
                                <td class="mono">{{ $item->no_rekamedis ?: '-' }}</td>
                                <td><strong>{{ $item->nama_pasien ?: '-' }}</strong></td>
                                <td>{{ $item->ruangan ?: '-' }}</td>
                                <td><span class="badge badge-primary">{{ $item->gol_darah ?: '-' }}</span></td>
                                <td>{{ $item->gol_rhesus ?: '-' }}</td>
                                <td class="mono">{{ $item->barcode ?: '-' }}</td>
                                <td>
                                    @php
                                        $hasil = $item->hasil ?? '';
                                        $badgeClass = 'badge-secondary';
                                        if ($hasil === 'COMPATIBLE') $badgeClass = 'badge-success';
                                        elseif (strpos($hasil, 'INCOMPATIBLE') !== false) $badgeClass = 'badge-danger';
                                        elseif ($hasil) $badgeClass = 'badge-warning';
                                    @endphp
                                    <span class="badge {{ $badgeClass }}">
                                        @if($hasil === 'COMPATIBLE')<i class="fas fa-check-circle"></i>
                                        @elseif(strpos($hasil, 'INCOMPATIBLE') !== false)<i class="fas fa-times-circle"></i>
                                        @elseif($hasil)<i class="fas fa-exclamation-circle"></i>@endif
                                        {{ $hasil ?: '-' }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    @php
                                        $sts = $item->status ?? '';
                                        $sc = 'badge-warning';
                                        if ($sts === 'SELESAI' || $sts === 'COMPATIBLE') $sc = 'badge-success';
                                        elseif ($sts === 'BATAL') $sc = 'badge-danger';
                                    @endphp
                                    <span class="badge {{ $sc }}">
                                        @if($sc === 'badge-success')<i class="fas fa-check-circle"></i>
                                        @elseif($sc === 'badge-danger')<i class="fas fa-ban"></i>
                                        @else<i class="fas fa-clock"></i>@endif
                                        {{ $sts ?: '-' }}
                                    </span>
                                </td>
                                <td>
                                    @php $sp = $item->status_pemberian ?? ''; @endphp
                                    @if($sp === 'transfusi')
                                        <span class="badge badge-success"><i class="fas fa-check-circle"></i> Transfusi</span>
                                    @elseif($sp)
                                        <span class="badge badge-info"><i class="fas fa-info-circle"></i> {{ $sp }}</span>
                                    @else
            ...
            @if(count($data) === 0)
                <div class="empty-state" style="padding:60px 20px;">
                    ...
                    <p style="font-size:14px;color:var(--text-muted);">Belum ada data permintaan darah.</p>
                </div>
            @endif
    ...
    <script>
        $(document).ready(function () {
            var table = $('#historyTable').DataTable({
                ...
                dom: 'rt<"bottom"lip>',
                language: {
                    search: '', searchPlaceholder: 'Cari dalam tabel...',
