    ...
    <style>
        ...
        * {
        ...
        .pagination-wrapper nav span[aria-disabled="true"] span {
            ...
            font-size: 13px; font-weight: 600; transition: all 0.2s; text-decoration: none; margin: 0 2px;
        }

        .pagination-wrapper nav span[aria-current="page"] span {
            background: var(--accent); color: white; box-shadow: 0 2px 8px rgba(99, 102, 241, 0.25); border: 1px solid var(--accent);
        }

        .pagination-wrapper nav a { background: white; color: var(--text-secondary); border: 1px solid var(--border); }
        .pagination-wrapper nav span[aria-disabled="true"] span { background: var(--bg-main); color: var(--text-muted); border: 1px solid var(--border); }
        .pagination-wrapper nav a:hover { background: var(--accent-light); border-color: var(--accent); color: var(--accent); }
                        ...
                        <tbody>
                            @forelse($userRoles as $index => $ur)
                                <tr>
                                    ...
                                    <td>
                                        <span style="background: var(--primary-light); color: var(--primary-dark); padding: 4px 10px; border-radius: 50px; font-size: 12px; font-weight: 600;">
                                            {{ $ur->jabatan ? $ur->jabatan->nm_jbtn : '-' }}
                                        </span>
                                    </td>
                                    <td>
                                        <form action="{{ route('web-user-roles.destroy', $ur->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus role user ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" style="text-align: center; color: var(--text-muted); padding: 32px 0;">Belum ada data mapping user role.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="modal">
            ...
            <form action="{{ route('web-user-roles.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="role_id" id="role_id" class="form-control select2-static" style="width: 100%;" required>
