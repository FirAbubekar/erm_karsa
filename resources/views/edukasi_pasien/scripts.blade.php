...
inputNoRm.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();searchPatient()}});

async function searchPatient(){
    const noRm=inputNoRm.value;
    if(!noRm) return showError('Silakan masukkan No. RM');
    btnSearch.disabled=true;btnSearch.innerHTML='Mencari...';
    try{
        ...
        if(res.ok){
            ...
            if(history.length>0){
                history.forEach((reg,i)=>{
                    ...
                    row.innerHTML=`<td>${i+1}</td><td>${reg.tgl_registrasi}</td><td>${reg.no_rawat}</td><td><span class="badge badge-blue" style="cursor:pointer">Pilih</span></td>`;
        ...
        }else{showError(result.error||'Terjadi kesalahan')}
    }catch(e){console.error(e);showError('Gagal menghubungi server')}
    finally{btnSearch.disabled=false;btnSearch.innerHTML='Cari'}
}
...
function populateAssessmentForm(data) {
    ...
    if(data.bahasa_lainnya) { document.getElementById('bahasa-lainnya').value = data.bahasa_lainnya; document.getElementById('bahasa-lainnya').style.display = 'block'; }
    ...
    if(data.pendidikan_lainnya) { document.getElementById('pendidikan-lainnya').value = data.pendidikan_lainnya; document.getElementById('pendidikan-lainnya').style.display = 'block'; }
    ...
    if(data.hambatan_lainnya) { document.getElementById('hambatan-lainnya').value = data.hambatan_lainnya; document.getElementById('hambatan-lainnya').style.display = 'block'; }
    setRadio('kesediaan_menerima', data.kesediaan_menerima);

    // Section B
    setCheckboxes('rencana_kebutuhan[]', data.rencana_kebutuhan);
    if(data.rencana_lainnya) { document.getElementById('rencana-lainnya').value = data.rencana_lainnya; document.getElementById('rencana-lainnya').style.display = 'block'; }
    ...
    if(data.ttd_pasien_wali) {
        ...
        if(sigBox) {
            ...
            if(!img) { img = document.createElement('img'); img.style.cssText='max-width:100%;max-height:100%;object-fit:contain'; sigBox.appendChild(img); }
...
function populateImplementations(implementations) {
    ...
    implementations.forEach(impl => {
        ...
        if(sebutkanInput && impl.nama_topik) {
            const originalTopik = row.dataset.originalTopik || '';
            const prefix = originalTopik.split(/\([sS]ebutkan\)/)[0];
            if(impl.nama_topik.startsWith(prefix)) {
                let detail = impl.nama_topik.substring(prefix.length);
        ...
        if(impl.ttd_pasien) {
            ...
            if(ttdBox) {
                ...
                if(!img) { img = document.createElement('img'); img.style.cssText='max-width:100%;max-height:100%;object-fit:contain'; ttdBox.appendChild(img); }
...
function resetAssessmentForm() {
    ...
    document.querySelectorAll('input[name="bahasa[]"], input[name="hambatan_edukasi[]"], input[name="rencana_kebutuhan[]"]').forEach(cb => cb.checked = false);
    ['perlu_penerjemah','baca_dan_tulis','pendidikan','nilai_budaya','gaya_pembelajaran','literasi_kesehatan','kesediaan_menerima'].forEach(name => {
...
document.querySelectorAll('[data-toggle-other]').forEach(cb=>{
    ...
    if(target){
        cb.addEventListener('change',()=>{target.style.display=cb.checked?'block':'none'});
...
document.querySelectorAll('[data-toggle-other-radio]').forEach(radio=>{
    ...
    if(target){
        document.querySelectorAll(`input[name="${groupName}"]`).forEach(r=>{
            r.addEventListener('change',()=>{target.style.display=radio.checked?'block':'none'});
        });
    }
});
...
function openSignatureModal(targetId){
    ...
    setTimeout(()=>{resizeSigCanvas();sigPad.clear();document.getElementById('signature-start-hint').style.display='flex';sigCanvas.focus()},50);
}

sigPad.onBegin=()=>{document.getElementById('signature-start-hint').style.display='none'};

document.getElementById('close-signature-modal').addEventListener('click',()=>{sigModal.style.display='none';document.body.style.overflow=''});
document.getElementById('clear-signature').addEventListener('click',()=>{sigPad.clear();document.getElementById('signature-start-hint').style.display='flex'});
...
document.getElementById('btn-save-edukasi').addEventListener('click', async ()=>{
    ...
    try {
        ...
        btn.disabled = false;
        btn.innerHTML = '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:18px"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Simpan Data Edukasi';
    }
});
...
async function saveImplementationRow(btn) {
    ...
    let namaTopik = '';
    if (isCustom) {
        const topikInput = topikCell ? topikCell.querySelector('input') : null;
        namaTopik = topikInput ? topikInput.value : '';
    } else {
        ...
        if (sebutkanInput) {
            ...
            namaTopik = originalTopik.replace(/\([sS]ebutkan\)/, `(${detailVal})`);
        } else {
            namaTopik = topikCell ? topikCell.textContent.trim() : '';
        }
    }
    ...
    // TTD Pasien
    const ttdPasienBox = row.querySelector('[id^="ttd-pasien-"]');
    const ttdPasienData = ttdPasienBox && ttdPasienBox.dataset.signatureData ? ttdPasienBox.dataset.signatureData : null;
    ...
    btn.innerHTML = '<svg class="animate-spin" style="width:14px;height:14px" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" opacity=".25"></circle><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" opacity=".75"></path></svg>';
